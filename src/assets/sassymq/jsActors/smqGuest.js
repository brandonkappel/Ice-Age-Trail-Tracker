...
function generateGuestActor() {
    ...
    smqGuest.sendReply = function(replyPayload, msg) {
        ...
        smqGuest.GetAllSegments = function() {
            smqGuest.GetAllSegments('{}');
        }

        smqGuest.GetAllSegments = function(payload) {
            payload = smqGuest.stringifyValue(payload);
            var id = smqGuest.createUUID();
            var deferred = smqGuest.waitingReply[id] = smqGuest.defer();
            if (smqGuest.showPingPongs) console.log('Get All Segments - ');
            smqGuest.client.send('/exchange/guestmic/crudcoordinator.crud.guest.getallsegments', { "content-type": "text/plain", "reply-to":"/temp-queue/response-queue", "correlation-id":id }, payload);
